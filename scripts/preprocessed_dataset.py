...
class PreprocessedDataset(dataset.DatasetMixin):
    ...
    def __init__(self, path, num_class, num_view, img_size=(384, 512), random=True):
        self.base = path
    ...
    def load_orig_data(self, c_idx, v_idx):
        ...
        return rgb, depth, mask, dist

    def get_example(self, i):
        img_size = self.img_size
        c_i = i // self.n_view
        v_i = i % self.n_view
        img_rgb, img_depth, mask, dist = self.load_orig_data(c_i, v_i)
        ...
        return img_rgb, img_depth, label.astype(np.int32), pose
