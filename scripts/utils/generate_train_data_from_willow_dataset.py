...
def main():
    ...
    for i in range(num_class):
        ...
        for j in range(num_view):
            ...
            organized_pc = np.zeros(3 * img_height * img_width).reshape(img_height, img_width, 3)
            dist_map = np.zeros(3 * img_height * img_width).reshape(img_height, img_width, 3)
            for i in range(img_height):
                for j in range(img_width):
                    for k in range(3):
                        organized_pc[i][j][k] = pc.pc_data[i * img_width + j][k]
            ...
            # save organized point cloud as numpy array
            np.save(os.path.join(c_path, 'pc_' + v_idx + '.npy'), organized_pc)
            print 'saving : ' + os.path.join(c_path, 'pc_' + v_idx + '.npy')
            # print pose_mat
            # print pos
            # print rot
